#ifndef ENCRYPT_HPP
#define ENCRYPT_HPP

#include <array>
#include <stdio.h>
#include <vector>

//"You get what you get, and don't throw a fit." -Wyatt 2017

class encrypt {
public:

	encrypt(int rounds, int words, bool mode);

	void read_key(std::vector<unsigned char> & key_in);

	void key_expansion();

	std::array<unsigned char, 16> encrypt_block(std::array<unsigned char, 16> & current_input);

private:

	//LOGIC FUNCTIONS

	void r_key(int round);
	void sub_bytes();
	void shift_rows();
	void mix_columns();

	//HELPER FUNCTIONS

	unsigned char xtime(unsigned char & x);
	

	//PRIVATE MEMBERS

	bool encryption_mode;

	int num_rounds;
	int num_words;

	unsigned char round_key[240];

	unsigned char key[32];
	
	unsigned char previous_chunk[4][4];
	unsigned char current_chunk[4][4];

	//AES SBOX - Pregenerated using Rijndael's Finite Field, with the final values taken from Wikipedia

	unsigned char s_box[256] =
	{
		
	};

	//Round Constats - Only the first some of these constants are actually used – up to rcon[10] for AES-128 (as 11 round keys are needed), up to rcon[8] for AES-192, up to rcon[7] for AES-256.
	//Final values taken from Wikipedia

	unsigned char round_constant[11] =
	{
		0x8D, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36
	};

};

#endif